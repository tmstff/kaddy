...
```sh
make docker-build docker-push IMG=<some-registry>/kaddy:tag
```
...
```sh
make deploy IMG=<some-registry>/kaddy:tag
```
...
```sh
make build-installer IMG=<some-registry>/kaddy:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/kaddy/<tag or branch>/dist/install.yaml
```
